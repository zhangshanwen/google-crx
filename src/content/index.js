...
async function MobileShow() {
    ...
    const packageName = $('div[class^="line"]').eq(2).text();
    ...
    return {package_name: packageName, title: title.val(), short_desc: short_desc.val()};
}
...
async function GetABTestDetail(report_id) {
    ...
    $("div[class='particle-table-row']").each(function (i, n) {
        ...
        const upper_endpoint = $(n)
            ...
            .text();
        const imgName = $(n).next().find("div[class^='_ngcontent']")
        const imgType = imgName.parent().parent().parent()
        let imgList = []
        $(n).next().find("img").each(function (i, n) {
        ...
        variantInfo["lower_endpoint"] = parseFloat(lower_endpoint ? lower_endpoint : 0) / 100;
        variantInfo["upper_endpoint"] = parseFloat(upper_endpoint ? upper_endpoint : 0) / 100;
